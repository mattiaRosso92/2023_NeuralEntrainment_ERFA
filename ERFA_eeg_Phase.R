...
dataPhase <- read_excel('/Users/mattiaipem/Documents/IPEM/Projects/ Lou _ Entrainment/ERFA_Metronomes/Data/Experiment/ERFA_eeg_Phase.xlsx')
# Convert variables to factors
dataPhase$Component <- factor(dataPhase$Component)
dataPhase$Direction <- factor(dataPhase$Direction)
# Re-order factors, for interpretation
dataPhase$Component <- relevel(dataPhase$Component , 'Audio')
contrasts(dataPhase$Component) #control needs to be 0 -> intercept provides the baseline
...
# dataPhase <- subset(dataPhase, Subject != 6 & Subject != 4 & Subject != 10 & Subject != 12 & Subject != 13 & Subject != 16 & Subject != 20)


### Model fitting ###
model <- lmer(Integral ~ Component*Direction + (1 | Subject) ,
