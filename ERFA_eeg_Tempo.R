...
dataTempo <- read_excel('/Users/mattiaipem/Documents/IPEM/Projects/ Lou _ Entrainment/ERFA_Metronomes/Data/Experiment/ERFA_eeg_Tempo.xlsx')
# Convert variables to factors
dataTempo$Component <- factor(dataTempo$Component)
dataTempo$Direction <- factor(dataTempo$Direction)
# Re-order factors, for interpretation
dataTempo$Component <- relevel(dataTempo$Component , 'Audio')
contrasts(dataTempo$Component) #control needs to be 0 -> intercept provides the baseline
...
stepn1 <- as.numeric(names(dataTempo))[4]; # get 1st time value (first index after factors)
# Compute downsampling-factor (if any)
dwn <- as.numeric(names(dataTempo)[5]) - as.numeric(names(dataTempo)[4])

# Remove outlier: Subject 6 (missing data) and eventual outliers
dataTempo <- subset(dataTempo, Subject != 6)
# dataTempo <- subset(dataTempo, Subject != 6 & Subject != 4 & Subject != 10 & Subject != 12 & Subject != 13 & Subject != 16 & Subject != 20)
 
# Convert to longitudinal data format: person, (other factors) , period
dataTempo = melt(dataTempo , id = c("Subject","Component","Direction"))
...
### Model fitting ###
#Fit quadratic model
model.quad <- lmer(InstFrex ~ (ot1+ot2)*Component*Direction +
                   (ot1+ot2 | Subject) + #random effect of participant
                   (ot1+ot2 | Subject:Component:Direction) , #...and interactions with factors
...
ggplot(data = dataTempo , aes(x = Time, y = InstFrex, color = Direction, shape = Component)) +
  facet_grid(rows = vars(Direction) , cols = vars(Component)) +
  geom_point() +
...
ggplot(data = dataTempo , aes(x = Time, y = fitted(model.quad), color = Direction, shape = Component)) +
  facet_grid(rows = vars(Direction) , cols = vars(Component)) +
  geom_point() +
  scale_color_brewer(palette="Set1") + #manually set color mapping
  scale_shape_manual(values=c(16, 1))+  #manually set shape mapping
...
## Visualize per subject; x = ot1 because it's the linear increase in the model
#paste("0",dataTempo$Dyad[recurrence$Dyad<10], sep = "", collapse = NULL) # Re-label dyads, for proper order
#ggplot(dataTempo = dataTempo , aes(x = ot1, y = Recurrence , color = Perspective , shape = Coupling)) +  
#  geom_point() +
#  geom_line(aes(y = fitted(recurrence.quad))) +
#  scale_color_brewer(palette="Set1") + #manually set color mapping 
#  scale_shape_manual(values=c(16, 1))+  #manually set shape mapping
#  facet_wrap(~Dyad) + #this is how you divide by subject
#  theme(axis.text.x = element_blank()) +
#  xlab("Expected relative phase") +
#  ylab("Recurrence score (log scale)")
...
# # Generate table
# table.model <- tab_model(model.quad , show.se = TRUE ,show.ci = FALSE ,
#           show.re.var = FALSE , show.obs = FALSE ,
#           show.r2 = FALSE , show.icc = FALSE , show.ngroups = FALSE ,
#           p.style = "numeric_star" ,
#           pred.labels = c("(Intercept)","Time","Time^2","Perspective","Coupling","Time:Perspective","Time^2:Perspective","Time:Coupling","Time^2:Coupling","Perspective:Coupling","Time:Perspective:Coupling","Time^2:Perspective:Coupling"))
# # Save table
# ggsave(grid.table(table.model), filename = "~/Users/mattiaipem/Desktop/trytab.png")
